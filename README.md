...
3. 導師按「重新整理教師」，對待審申請選「批准」或「拒絕」。批准後該班出現在共同教師的班級列表；可進入「記分」使用單筆記分，再按「查看分數紀錄」前往紀錄頁。
...
1. 導師或已批准共同教師，在班級卡片選「記分」。記分頁只呈現新增表單。
...
4. 按「查看分數紀錄」跳轉至獨立紀錄頁，可看全班紀錄，或選一位學生看總分及明細。總分從 0 累計，可為負數；每頁 25 筆，最新在前。按「返回記分」繼續填寫；兩頁切換保留表單草稿與學生篩選，重新載入最新資料。
5. 學生完成首次改密碼後，在個人頁看到自己的總分、原因、待補標記、教師姓名與時間；按「更新分數紀錄」取得最新結果。

送出按鈕與記分／紀錄切換按鈕在處理時停用；同一表單內容失敗後重試沿用操作識別，若伺服器其實已完成，不會再次計分。記分／紀錄兩頁切換保留重試識別；重新整理網頁、返回我的班級或登出會失去表單暫存。不確定結果時先查看紀錄。後端以班級、操作者及 UUID 去重，相同識別卻換內容會拒絕。不同新操作即使內容相同也可再次記分。
...
更新環境時執行 `backend/manage.py migrate`（`CM_DEBUG=1`），套用 0010；既有單筆分數、查核及總分保留。

1. 在「記分」將「記分模式」切成「批次記分」，按座號勾選學生或全選／清除，設定共同的加扣種類、分數與原因，再按「送出批次記分」。每位學生各有獨立紀錄，同次操作共用批次識別，紀錄卡顯示「同批記分」。
2. 進入「查看分數紀錄 → 整理待補原因」。導師可補全班，共同教師只列出自己建立的待補紀錄。每頁 25 筆，可選一筆或全選本頁，輸入非空白的統一原因後儲存；換頁會清除選取。
3. 任一學生已刪除、紀錄已補完或無權限，整批都不儲存；錯誤會指出失效的選取項目。重新整理並調整選取後再送出。

補原因不改分數、種類或建立者，每筆保留 before／after 查核；分數總分不變。批次記分的紀錄、總分及查核也在同一交易完成。相同操作重試不重複寫入，相同識別改內容會拒絕。回應遺失時可保留原選取及內容重試，頁面切換保留暫存；重新整理網頁、返回我的班級或登出則清除暫存。一般修改／軟刪除與完整查核介面留待 07。

永久刪除學生會清除該生批次紀錄及補原因查核，其他學生保留；永久刪班會清除全部所屬紀錄。批次中部分學生已刪除時，重送原整批會拒絕，避免補建部分紀錄。

自動流程見 `e2e/scores.spec.ts`，桌面／手機截圖在 `.local/06-batch-desktop.png`、`06-batch-mobile.png`、`06-pending-desktop.png`、`06-pending-mobile.png`。
...
- 已實作任務 01、01a、02、03、03a、04、05、06；修改／軟刪除、點數與餵食依後續任務完成。
